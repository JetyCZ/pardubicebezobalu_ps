...
<form id="configuration_form" action="index.php?controller=AdminProducts&token={$prod_token|escape:'html':'UTF-8'}"   method="post"    class="defaultForm form-horizontal productorderspro">
<tr class=" odd">
...
{if $nka.id_supplier==$product->id_supplier && $nka.id_product==Tools::getValue('id_product')}
...
:Price: {displayPrice price=ProductOrderPro::getSupplierPrice({$nka.id_product_supplier|escape:'html':'UTF-8'})}
</option>
{/if}
{/foreach}
</select>
...
<td class="">
<input type="text" name="comment" class="edit_product_quantity" value="{$product->product_quantity|escape:'html':'UTF-8'}">
</td>
<!--<td class="">
...
<input type="submit" name="submitAddproduct" value="{l s='Add Product' mod='productorderspro'}" {literal}onclick="if (confirm('Ajouter ce produit ?')){return true;}else{event.stopPropagation(); event.preventDefault();};"{/literal} title="add product" class="edit btn btn-default">
</div>
</div>
</td>
</tr>
