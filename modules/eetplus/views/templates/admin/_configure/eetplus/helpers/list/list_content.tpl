			...
			{block name="open_td"}
				<td
					...
					{if (!isset($params.position) && !$no_link && !isset($params.remove_onclick))}
						onclick="document.location = '{$current_index|escape:'html':'UTF-8'}&amp;{$identifier|escape:'html':'UTF-8'}={$tr.$identifier|escape:'html':'UTF-8'}{if $view}&amp;view{else}&amp;update{/if}{$table|escape:'html':'UTF-8'}&amp;token={$token|escape:'html':'UTF-8'}&amp;id_eetplus={$tr.id_eetplus}'">
					{else}
					>
				{/if}
			{/block}
			{block name="td_content"}
				...
				{if isset($tr.$key)}
					...
					{elseif isset($params.type) && $params.type == 'editable'}
						<input type="text" name="{$key}[{$tr.$identifier}]" value="{$tr.$key|escape:'html':'UTF-8'}" class="{$key}" />
					...
					{elseif isset($params.type) && $params.type == 'crude'}
					     <select name="{$key}[{$tr.$identifier}]" '>{$tr.$key}</select>
                    {elseif $key =='fik'}
                      <span class = "color_field" style="background-color:#{$tr.fikcolor};color:white;"> {$tr.fikmessage} </span>
					{else}
                        {if isset($params.color) && isset($tr[$params.color])}
                               {$tr.$key|escape:'html':'UTF-8'|truncate:20}  
                        {else}
