...
function remtransprepare()
{ remtransform.verbose.value = configform.verbose.checked;  
}
...
problem is solved.<p>
Lower on this page you can also repair the cache flags. Consider also emptying your caches (another option in 
the Prestools menu).<p>
<?php
$index = 0;
...
echo '<tr><td>Field</td><td>shopgroup</td><td>shop</td><td>Value</td><td></td><td>Comment</td></tr></thead><tbody>';
  ...
  */
  ...
  /cache/smarty/compile and theme cache directories.</td></tr>';
  echo '<tr><td style="text-align:center"><form name=cacheform action="cleanup-proc.php" method=post target=tank onsubmit=cfprepare()>';
  ...
  echo '<br><table class="triplemain" style="margin-top:-15px">';
  echo '<thead><tr><td style="text-align:center"><b>Remove deleted languages</b></td></tr></thead>';
  echo '<tbody><tr><td>When you delete languages Prestashop often leaves translations in the tables for
  the erased languages. Pressing this button will immediately delete those translations.</td></tr>';
  echo '<tr><td style="text-align:center"><form name=remtransform action="cleanup-proc.php" method=post target=tank onsubmit=remtransprepare()>';
  echo '<input type=hidden name="subject" value="removetranslations" ><input type=hidden name=verbose>';
  echo '<input type=submit value="remove unused translations"></form></td></tr></table>';
...
echo '<hr><table class="triplemain"><tr><td style="text-align:center"><b>Retrieve backup database</b></td></tr><tr><td>';
  $dres = dbquery("SELECT DATABASE() AS mydb");
  ...
  { echo '<table><tr><td><form name="restoreform" action="shop-rescue-proc.php" method=post target=tank onsubmit="return checkrestore();">Select an empty database in which to restore the backup: ';
	...
	if(!file_exists("../ajax.php")) echo "You can only run this function from a directory below your admin!";
    else if(!($files = scandir($backuppath))) echo "No backup directory found";
    else if(sizeof($files) <= 3) echo "Backup directory is empty";
	else 
