...
//Not true in multishop  if(mysqli_num_rows($res) != 1) { echo "Rowcount error. This shouldn't happen."; exit(0); } 
  ...
  { $tmp .= ' Copy <input type=radio name=sourcetype value="all" checked onchange="changeMethod(0)"> all '.mysqli_num_rows($bres).' combinations. ';
	...
	$tmp .= '<span id=combinationlist style="display:none">';
  }
  else
  ...
  { $star = "";
    ...
    if($task == "prodcopycombis")	
       $tmp .= '<input type=checkbox name="combis[]" value="'.mysort($parow["id_product_attribute"]).'" checked>'.$parow["nameblock"]." ".$parow["id_product_attribute"].$star."<br>";
	else
       $tmp .= '<input type=checkbox name="combis[]" value="'.mysort($parow["attr_block"]).'">'.$parow["nameblock"]." ".$parow["id_product_attribute"].$star."<br>";
  }
  if($task == "prodcopycombis")	
    $tmp .= '</span>';
  $tmp = $tmp."\n";
  ...
  $aquery .= " GROUP BY pc.id_attribute ORDER BY a.id_attribute_group";
