...
{ global $combiblock, $combis, $shoprows, $stockrows, $whrows, $id_lang, $noreplace, $conn, $prodctr;
  global $input, $affected_shops, $timelimit, $modified_products_for_cpr, $starttime;
  ...
  { if(in_array($combi, $prodcombis))
	...
	{ 	$query = "INSERT INTO "._DB_PREFIX_."product_attribute SET id_product='".$id_product."',";
		...
		$query .= "supplier_reference='',location='".$combiblock[$combi]["location"]."',ean13='',upc='',";
		if(in_array("isbn",$optional_pafields)) // _PS_VERSION_ >= "1.7.0"
			$query .= "isbn='',";
		if(in_array("date_upd",$optional_pafields)) // Vivianne fix
			$query .= "date_upd=now(),";
	    $query .= "wholesale_price='".$combiblock[$combi]["wholesale_price"]."',";
...
update_shop_index(15);  /* in ps_sourced_code.php. The number is the number of seconds that it is allowed to run. */
