...
if(!isset($_GET["id_image"])) colordie("No image specified"); else $id_image = $_GET["id_image"];
if($action == "generate")
{ if(!isset($_GET["imageformat"])) colordie("No format specified"); else $imageformat = $_GET["imageformat"];
}
...
{ /* get the image format */
  $query = 'SELECT name,width,height FROM `'._DB_PREFIX_.'image_type`';
  $query .= ' WHERE products=1 AND id_image_type="'.$imageformat.'"';
  $res = dbquery($query);
  ...
  echo "width=".$width.", height=".$height." targetwidth=".$targetwidth.", targetheight=".$targetheight."<br>";
