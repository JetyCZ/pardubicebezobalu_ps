...
{ str = '';
  ...
  { $reverse = "false";
    ...
    { $sorttxt = "";
	  ...
	  if($i == 0) /* id */
      {   echo "<td><a href='".preg_replace("/^..\//","",$triplepath)."index.php?controller=AdminProducts&id_product=".$myvalue."&updateproduct' title='".$datarow['originalname']."' target='_blank'>".addspaces($id_product)."</a>";
	  ...
	  else if(($fields[$i]=="active") || ($fields[$i]=="onsale") || ($fields[$i]=="onlineonly"))
      ...
      { $iquery = "SELECT id_image,cover FROM ". _DB_PREFIX_."image WHERE id_product='".$datarow['id_product']."' ORDER BY position";
 	  ...
 	  { $mquery = "SELECT id_manufacturer,name FROM "._DB_PREFIX_."manufacturer ORDER BY name";
		...
		{ $selected = '';
		  ...
		  echo '<option value="'.$mrow['id_manufacturer'].'"'.$selected.'>'.str_replace("'","\'",$mrow['name']).'</option>';
		}   
		echo "</select></td>";
	  }
      else if ($fields[$i] == "name")
		  echo '<td><input style="width:300px" name="'.$field_array[$fields[$i]][0].'" value="'.$myvalue.'"></td>';
