...
function update_shop_index($maxtime)
{ global $conn, $verbose;
  if($maxtime > 25)
...
//	'pa_supplier_reference' => get_configuration_value('PS_SEARCH_WEIGHT_REF'), // included in supplier_references
  ...
  if($verbose == "true")
    echo "<br>Re-indexing for max ".$maxtime." seconds. Starting at ".date("h:m:s",$starttime)."</br>";
  while(true)
  { $query = 'SELECT '.$p_fields.'
			...
			ORDER BY ps.id_product, ps.id_shop, pl.id_lang
			LIMIT '.$batchsize;

/* Note: shops that have inactive languages might consider adding "AND pl.active!=0" to the WHERE clause */
	...
	{ /* Prestashop assumes that we finish indexing everything. In Prestools however, we index only a few seconds 
	  ...
	  echo $product["id_product"]."-".$product["id_shop"].",";
	  if(!(($x-1)%16)) echo " <br>";
	  ...
	  { if(isset($product_array[$prow["word"]]))
		...
		{ $uquery = 'DELETE FROM '._DB_PREFIX_.'search_index';
		  $uquery .= ' WHERE id_word='.$prow["id_word"].' and id_product='.$product['id_product'];
		  $ures = dbquery($uquery);
		}
	  }
